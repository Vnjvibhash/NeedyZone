...
import { cctvProducts, getProductById } from "@/lib/products-data"
import { ArrowLeft, Check, Phone, FileText, Download } from "lucide-react"

export function generateStaticParams() {
  return cctvProducts.map((product) => ({ id: product.id }))
}

export default async function CCTVProductDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  const product = getProductById(id)

  if (!product || product.category !== "cctv") {
    notFound()
  }

  const relatedProducts = cctvProducts.filter((p) => p.id !== product.id).slice(0, 3)
        ...
        <div className="border-b border-border bg-muted/30">
          <div className="container mx-auto px-4 py-4 lg:px-8">
            <div className="flex items-center gap-2 text-sm">
              ...
              <Link href="/products/cctv" className="text-muted-foreground hover:text-foreground">
                CCTV Cameras
              </Link>
              <span className="text-muted-foreground">/</span>
            ...
            <Tabs defaultValue="specifications" className="w-full">
              ...
              <TabsContent value="applications">
                <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
                  {["Residential Homes", "Office Buildings", "Retail Stores", "Warehouses & Factories"].map((app) => (
                    <Card key={app}>
